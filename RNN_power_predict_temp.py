...
df = pandas.read_csv('household_power_consumption', sep=';',na_values=['?']) # convert missing value -> ? to NAN
...
df = df[:limit_rows]# for more accurate result increase the input row size
# Get the header info
...
predicted = rnn(Variable(torch.FloatTensor(X_test)).cuda()).cpu()

#print(explained_variance_score(predicted.data.numpy(),y_test.data.numpy()))

#fig = plt.figure(X_test)
#ax = fig.add_subplot(111)
plt.plot(y_test,marker='.',color='red',label="Actual")
plt.plot(predicted.data.numpy(),marker='.',color='green',label="Predicted")
plt.legend()
#plt.show()
